...
async function getData(): Promise<{ prayer: Prayer; prayerIndex: PrayerIndex }> {
  try {
    ...
    // Используем текущее время для получения действительно случайной молитвы
    // Это гарантирует, что при каждом заходе будет разная молитва
    const timestamp = Date.now();
    const randomSeed = timestamp % prayerIndex.totalPrayers;
    const prayerUrl = prayerIndex.prayers[randomSeed]?.url;
